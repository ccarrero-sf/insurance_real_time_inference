...
This script uploads all Python source files from task_graph_stage/src/ to
@CC_INSURANCE_PIPELINE.PIPELINE_CODE_STAGE.CODE_STAGE so that Snowflake Tasks
can reference them via imports.

Must be run BEFORE deploying the DAG (deploy_dag.py).
After initial deployment, run this script whenever source code changes —
the updated files take effect on the next DAG run without redeploying the DAG.
...
# Source files live in task_graph_stage/src/, one level up from scripts/
...
STAGE_SCHEMA = "PIPELINE_CODE_STAGE"
