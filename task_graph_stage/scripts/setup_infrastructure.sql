...
-- PIPELINE_CODE_STAGE schema: tasks, stages, DAG artifacts
CREATE SCHEMA IF NOT EXISTS IDENTIFIER($DB_NAME || '.PIPELINE_CODE_STAGE');
...
CREATE STAGE IF NOT EXISTS IDENTIFIER($DB_NAME || '.PIPELINE_CODE_STAGE.CODE_STAGE')
    ENCRYPTION = (TYPE = 'SNOWFLAKE_SSE');

-- JOB_STAGE: ML Job payloads (used by @remote decorator)
CREATE STAGE IF NOT EXISTS IDENTIFIER($DB_NAME || '.PIPELINE_CODE_STAGE.JOB_STAGE')
    ENCRYPTION = (TYPE = 'SNOWFLAKE_SSE');

-- ARTIFACTS_STAGE: model pickles, run artifacts, intermediate results
CREATE STAGE IF NOT EXISTS IDENTIFIER($DB_NAME || '.PIPELINE_CODE_STAGE.ARTIFACTS_STAGE')
    ENCRYPTION = (TYPE = 'SNOWFLAKE_SSE');

-- -------------------------
-- Image Repository (for ML Jobs on SPCS)
-- -------------------------
CREATE IMAGE REPOSITORY IF NOT EXISTS IDENTIFIER($DB_NAME || '.PIPELINE_CODE_STAGE.ML_IMAGE_REPO');
...
GRANT ALL ON SCHEMA IDENTIFIER($DB_NAME || '.PIPELINE_CODE_STAGE') TO ROLE IDENTIFIER($ROLE_NAME);
GRANT ALL ON SCHEMA IDENTIFIER($DB_NAME || '.DATA') TO ROLE IDENTIFIER($ROLE_NAME);

-- Grant usage on stages
GRANT READ, WRITE ON STAGE IDENTIFIER($DB_NAME || '.PIPELINE_CODE_STAGE.CODE_STAGE') TO ROLE IDENTIFIER($ROLE_NAME);
GRANT READ, WRITE ON STAGE IDENTIFIER($DB_NAME || '.PIPELINE_CODE_STAGE.JOB_STAGE') TO ROLE IDENTIFIER($ROLE_NAME);
GRANT READ, WRITE ON STAGE IDENTIFIER($DB_NAME || '.PIPELINE_CODE_STAGE.ARTIFACTS_STAGE') TO ROLE IDENTIFIER($ROLE_NAME);
...
SHOW STAGES IN SCHEMA IDENTIFIER($DB_NAME || '.PIPELINE_CODE_STAGE');
