"""
Pipeline Task Entry Points for Car Insurance ML DAG (ML Jobs version).

In this version, heavy compute tasks (ingest, train, inference) run as ML Jobs
directly on the compute pool via @remote-decorated functions in data_ops.py
and modeling.py. Those functions are used as DAG task definitions directly.
