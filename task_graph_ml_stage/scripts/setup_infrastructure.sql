...
-- -------------------------
-- CODE_STAGE: writable stage for StoredProcedureCall imports (sproc.register needs PUT access)
-- Source files are copied here from the Git repo stage before DAG deployment.
CREATE STAGE IF NOT EXISTS IDENTIFIER($DB_NAME || '.PIPELINE_STAGE.CODE_STAGE')
...
GRANT READ, WRITE ON STAGE IDENTIFIER($DB_NAME || '.PIPELINE_STAGE.CODE_STAGE') TO ROLE IDENTIFIER($ROLE_NAME);
