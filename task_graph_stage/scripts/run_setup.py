...
def run_setup(session: Session) -> None:
    ...
    statements = [
        ...
        # Schemas
        f"CREATE SCHEMA IF NOT EXISTS {DB_NAME}.PIPELINE_CODE_STAGE",
        f"CREATE SCHEMA IF NOT EXISTS {DB_NAME}.DATA",

        # Stages
        f"CREATE STAGE IF NOT EXISTS {DB_NAME}.PIPELINE_CODE_STAGE.CODE_STAGE ENCRYPTION = (TYPE = 'SNOWFLAKE_SSE')",
        f"CREATE STAGE IF NOT EXISTS {DB_NAME}.PIPELINE_CODE_STAGE.JOB_STAGE ENCRYPTION = (TYPE = 'SNOWFLAKE_SSE')",
        f"CREATE STAGE IF NOT EXISTS {DB_NAME}.PIPELINE_CODE_STAGE.ARTIFACTS_STAGE ENCRYPTION = (TYPE = 'SNOWFLAKE_SSE')",

        # Image Repository
        f"CREATE IMAGE REPOSITORY IF NOT EXISTS {DB_NAME}.PIPELINE_CODE_STAGE.ML_IMAGE_REPO",

        # Grants - database and schemas
        f"GRANT USAGE ON DATABASE {DB_NAME} TO ROLE {ROLE_NAME}",
        f"GRANT ALL ON SCHEMA {DB_NAME}.PIPELINE_CODE_STAGE TO ROLE {ROLE_NAME}",
        f"GRANT ALL ON SCHEMA {DB_NAME}.DATA TO ROLE {ROLE_NAME}",

        # Grants - stages
        f"GRANT READ, WRITE ON STAGE {DB_NAME}.PIPELINE_CODE_STAGE.CODE_STAGE TO ROLE {ROLE_NAME}",
        f"GRANT READ, WRITE ON STAGE {DB_NAME}.PIPELINE_CODE_STAGE.JOB_STAGE TO ROLE {ROLE_NAME}",
        f"GRANT READ, WRITE ON STAGE {DB_NAME}.PIPELINE_CODE_STAGE.ARTIFACTS_STAGE TO ROLE {ROLE_NAME}",
        ...
        f"SHOW STAGES IN SCHEMA {DB_NAME}.PIPELINE_CODE_STAGE",
    ]
