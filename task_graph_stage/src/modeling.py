...
def train_model_remote(session: Session, dataset_name: str, dataset_version: str):
    ...
    print ("Testin changes in this code after submit new version of the model and deployed -------!!!!!!")
...
def _build_inference_input(session: Session) -> "snowflake.snowpark.DataFrame":
    """
    Build enriched inference input by joining POLICIES + CUSTOMERS + Feature View.

    Returns a Snowpark DataFrame with exactly the columns the model expects
    ...
    # Select exactly the columns the model expects
    input_cols = NUMERIC_COLS + CATEGORICAL_COLS
...
def run_inference(session: Session, input_table: str, output_table: str) -> int:
    ...
    Builds an enriched input DataFrame by joining POLICIES + CUSTOMERS +
    Feature View to provide all the features the model expects.
    ...
    input_df = _build_inference_input(session)
    print(f"Running inference on {input_df.count()} enriched rows")
...
def run_inference_remote(session: Session, input_table: str, output_table: str) -> int:
    ...
    Self-contained: builds enriched input (POLICIES + CUSTOMERS + Feature View),
    loads the production model from registry, runs predictions, and saves results.
    All logic is inline for clean serialization to the compute pool.
    """
    ...
    # Build enriched input: POLICIES + CUSTOMERS + Feature View
    current_year = datetime.now().year
    policies_df = session.table(f"{pipeline_db}.{data_schema}.POLICIES")
    customers_df = session.table(f"{pipeline_db}.{data_schema}.CUSTOMERS").select(
        "CUSTOMER_ID", "AGE", "GENDER", "STATE",
    ...
    input_cols = numeric_cols + categorical_cols
    input_df = input_df.select(input_cols)
    row_count = input_df.count()
    print(f"Running inference on {row_count} enriched rows")

    # Load production model and run predictions
    registry = Registry(
        session=session,
        database_name=pipeline_db,
        schema_name=data_schema,
